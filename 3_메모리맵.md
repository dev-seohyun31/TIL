# 실제 개념
## 메모리 맵이란
메모리 맵이란, CPU가 쓰는 주소 공간에 어떤 하드웨어 자원이 어떤 주소 범위를 차지하는지 정의된 "하드웨어 주소 배치" 규칙입니다. 
메모리 맵은 MCU 내부 버스 구조와 주소 디코더 회로에 의해 하드웨어로 고정된 구조입니다. 아래와 같은 규칙들이 칩 설계 단계에서 이미 물리적으로 구현되어 있습니다. 
```
