...
즉, 통신 대상은 사람이 아니라, 
> MCU 내부 통신 Peripheral ↔ 외부 장치의 통신 Peripheral
...
```
CPU는 레지스터를 통해 주변장치를 설정·제어할 뿐이며,
실제 비트 송수신, 타이밍 생성, 버퍼링, 에러 검출은 하드웨어 Peripheral이 수행합니다.
### 제어 구조
> 기준: 누가 통신을 시작하고, 누가 타이밍과 흐름을 지배하는가
#### 1. P2P 구조 (UART)
...
송신자/수신자는 고정된 역할이 아니라, 순간적인 데이터 흐름 방향일 뿐입니다.
...
모든 노드가 송신 시작 권한을 가진 구조입니다.
중앙 Master 없이 동작하므로, 충돌 방지를 위해 다음 기능이 필요합니다.
1. 버스 중재: 누가 먼저 전송할 지 결정합니다. 
2. 버스 모니터링: 항상 버스 상태를 대기하다가, 자신이 낸 신호와 실제 버스를 비교합니다.
3. 자동 재시도: 중재에서 패배한 노드는 자동으로 대기했다가 재송신을 시도합니다.

긴급 이벤트를 중앙 통제 없이 즉시 네트워크 전체에 퍼뜨리기 위한 구조입니다. 
주로 **이벤트 즉시 전송**이 필요한 실시간 시스템에서 사용됩니다.
- '에어백 ECU'는 이벤트 발생 시, '즉시 브레이크 ECU'로 신호 전송
- 센서 이벤트 즉시 브로드캐스트
...
### 동기와 비동기
물리에서 통신에서의 동기/비동기는 비트 타이밍을 어떻게 맞추느냐가 기준입니다. 
(소프트웨어에서의 동기/비동기는 "제어 흐름" 관점도 MCAL에서 존재합니다)

#### 1. 비동기 통신 (UART, CAN, LIN, Ethernet)
공유 클록선을 가지지 않습니다. 
1. 약속으로 타이밍 맞추기 (UART)
    * 송신과 수신 측이 동일한 전송 속도(baud rate)를 미리 약속
    * start bit를 기준으로 타이밍을 동기화
    > 즉, 약속으로 타이밍을 맞춥니다.

2. 데이터에 클록 포함하 논리적 동기화 (CAN, LIN, Ethernet)
    * CAN의 경우, 내부에 Bit Timing Logic, Resynchronization, Sample Point 조절 하드웨어가 존재하여 수신측이 스스로 동기화합니다.
    * LIN의 경우, UART 기반이지만 Master가 Break 필드를 전송하면 Slave들이 이 신호로 Baud Rate를 재동기화하여 수신측이 동기화합니다.
    * Ethernet의 경우, PHY 내부의 Clock Recovery, PLL Lock 등을 사용하여 수신 데이터에서 클록을 복원합니다.


#### 2. 동기 통신 (SPI, I2C)
클록선을 물리적으로 공유합니다. 
Master가 클록을 생성해서 클록선으로 인가하면, Slave는 그 클록에 맞춰서 수신받은 비트들을 즉시 샘플링합니다. 
> 즉, 물리적인 신호로 타이밍을 직접 공유합니다.
...
* CS 기반 선택 (SPI)
    * Slave Select 선을 사용하여 물리적으로 대상을 지정합니다.
* 주소 기반(I2C, Ethernet)
    * 특정 장치를 명시적으로 지정하여 목적지가 명확합니다.
    * I2C: Slave 주소
    * Ethernet: MAC 주소, IP 주소
* 메시지 ID 기반 (CAN)
    * 모든 노든는 수신하고, 필요한 노드만 필터링합니다. (메시지 구독 개념)
 
#### 3. Multi-Node Bus (CAN, LIN, Ethernet)
여러 노드가 동일한 물리 버스를 공유합니다.
```
...
* 프레임 필터링이 필요합니다.
    * 네트워크 통신은 노이즈 환경에서 동작해야 하므로, Parity 수준을 넘어선 강력한 **프레임 신뢰성 구조**를 사용합니다.
...
#### 2. 직렬 통신 (UART, SPI, I2C, CAN, LIN, Ethernet)
