...
# 내가 이해한 바
# 개발자 관점에서 신경쓸 점
## RISC 기반 MCU 개발 시 주의할 점
* 메모리 접근 비용이 비싸기 문에 전역 변수를 남발하지 않는다.
* 파이프라인 전제가 강하고 분기가 많으면 성능이 급락할 수 있으므로, if-else 남발하지 않는다.
* 컴파일러 최적화의 영향이 매우 크므로, C 코드에서만 끝나지 않고 어셈블리/컴파일 결과를 한 번은 반드시 확인한다. 
