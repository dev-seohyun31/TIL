...
> AURIX의 CAN 모듈은 **MULTICAN** 이라 불리며,  
...
| 레지스터 | 역할 |
|---|---|
| `CAN_Nx_CCCR` | 노드 설정 모드 진입 / 초기화 |
| `CAN_Nx_NBTP` | Nominal Bit Timing (보드레이트 설정) |
| `CAN_Nx_DBTP` | Data Bit Timing (CAN FD 데이터 페이즈 속도) |
| `CAN_Nx_TXBC` | TX Buffer 설정 (메시지 RAM 주소, 크기) |
| `CAN_Nx_RXBC` | RX Buffer 설정 |
| `CAN_Nx_GFC` | Global Filter Config (수신 필터 기본 동작) |
| `CAN_Nx_IR` | 인터럽트 플래그 (TX 완료, RX 준비 등) |
| `CAN_Nx_IE` | 인터럽트 enable |
...
/* 1. 초기화 모드 진입 */
CAN_N0_CCCR.B.INIT = 1u;
CAN_N0_CCCR.B.CCE  = 1u;  /* 설정 변경 허용 */

/* 2. 비트타이밍 설정 (500kbps: BRP=4, TSEG1=15, TSEG2=4, SJW=4) */
CAN_N0_NBTP.B.NBRP   = 4u;
CAN_N0_NBTP.B.NTSEG1 = 15u;
CAN_N0_NBTP.B.NTSEG2 = 4u;
CAN_N0_NBTP.B.NSJW   = 4u;

/* 3. TX/RX 버퍼, 필터, 핀 라우팅 별도 설정 */
/* ... (생략, 수십 줄 필요) ... */

/* 4. 초기화 모드 해제 */
CAN_N0_CCCR.B.INIT = 0u;
```
...
/* TX 요청 */
CAN_N0_TXBAR.U = (1u << buffer_idx);

/* 완료 대기 */
while (!(CAN_N0_TXBTO.U & (1u << buffer_idx)));
```
